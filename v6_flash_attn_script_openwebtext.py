...
def get_batch(is_train=True):
    ...
    ix = torch.randint(num_dataset_articles, (batch_size,)).view(
        batch_size,
    )
    # print(ix)
    # find batch_size random starting indices in the data
    ...
    for i in ix:
        ...
        while len(article_ids) < block_size + 2:
            article_ids = encode(
                article + data[torch.randint(num_dataset_articles, (1,)).item()]["text"]
            )
        # print(f"len article ids: {len(article_ids)-block_size-1}")
        start_idx = torch.randint(0, len(article_ids) - block_size - 1, (1,)).item()
        x_list.append(
            torch.tensor(
                article_ids[start_idx : start_idx + block_size], dtype=torch.long
            )
        )
        y_list.append(
            torch.tensor(
                article_ids[start_idx + 1 : start_idx + block_size + 1],
                dtype=torch.long,
            )
        )
...
class SelfAttentionHead(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        self.register_buffer(
            ...
            torch.tril(
                ...
                - torch.tril(torch.ones(block_size, block_size), -sliding_window_len)
            ),
        )
    ...
    def forward(self, x):
        ...
        weights = (
            torch.matmul(q, k.transpose(-2, -1)) * C**-0.5
...
class MoEFFN(nn.Module):
    ...
    def forward(self, x):
        ...
        expert_outputs = torch.gather(
            expert_outputs, 3, topk_indices.unsqueeze(2).expand(-1, -1, C, -1)
        )  # (B, T, C, 2)
...
try:
    ...
    if load_previous:
        model.load_state_dict(torch.load("./models/model_v6_flash_attn.pth"))
    ...
    torch.save(model.state_dict(), "./models/model_v6_flash_attn.pth")
